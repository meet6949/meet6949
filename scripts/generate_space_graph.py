...
def generate_svg(contributions):
    ...
    def get_spaceship_x_at(time):
        t = time % spaceship_duration
        if t < spaceship_duration / 2:
            return (t / (spaceship_duration / 2)) * max_move_x
        else:
            return (1 - ((t - spaceship_duration / 2) / (spaceship_duration / 2))) * max_move_x
...
'''
    ...
    for idx, pos in enumerate(contrib_positions):
        delay = idx * 0.15
        ship_x = get_spaceship_x_at(delay)
        gun_x = ship_x + 76 # Align with the gun tip in the ship group

        target_x = pos["x"] - gun_x
        target_y = pos["y"] - spaceship_y

        # Laser beam
        svg += f'''        <g class="laser" style="animation-delay: {delay}s; --target-x: {target_x}px; --target-y: {target_y}px;">
            <circle cx="{gun_x}" cy="{spaceship_y}" r="3" fill="{LASER_COLOR}"/>
            <circle cx="{gun_x}" cy="{spaceship_y}" r="6" fill="url(#laserGlow)" opacity="0.6"/>
        </g>\n'''
